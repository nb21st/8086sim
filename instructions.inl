...
#define IMP_RM_IS_W {bits_rm_always_w, 0, 1}
...
/* Register|memory to segment register */ /* untested */
/* Segment register to register/memory */ /* untested */
...
/* CONTROL TRANSFER */
                                                                                                                   /* call = Call:              */
                                                                                                                   /* Direct within segment     */ /* untested */
    INST(call, BIN(ff, 11111111), IMP_D(0), IMP_W(1), MOD, BIN(02, 010), RM, DISP_LO, DISP_HI, _, _, _)            /*   Indirect within segment */
                                                                                                                   /* Direct intersegment       */ /* untested */
                                                                                                                   /* Indirect intersegment     */ /* untested */

                                                                                                                   /* jmp = Unconditional Jump:      */
                                                                                                                   /*    Direct within segment       */ /* untested */
                                                                                                                   /*    Direct within segment-short */ /* untested */
    INST(jmp, BIN(ff, 11111111), IMP_D(0), IMP_W(1), MOD, BIN(04, 100), RM, DISP_LO, DISP_HI, _, _, _)             /*    Indirect within segment     */
                                                                                                                   /*    Direct intersegment         */ /* untested */
                                                                                                                   /*    Indirect intersegment       */ /* untested */
...
ALT_INST(ret, BIN(c2, 11000010), IMP_S(0), IMP_W(1), DATA, DATA_IF_W, _, _, _, _, _, _)                            /*   Within segment adding immediate to SP */
    ...
    INST(je,     BIN(74, 01110100), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* je = Jump on equal|zero                 */
    INST(jl,     BIN(7c, 01111100), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jl = Jump on less|not greater or equal  */
    INST(jle,    BIN(7e, 01111110), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jle = Jump on less or equal|not greater */
    INST(jb,     BIN(72, 01110010), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jb = Jump on below|not above or equal   */
    INST(jbe,    BIN(76, 01110110), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jbe = Jump on below or equal|not above  */
    INST(jp,     BIN(7a, 01111010), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jp = Jump on parity|parity even         */
    INST(jo,     BIN(70, 01110000), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jo = Jump on overflow                   */
    INST(js,     BIN(78, 01111000), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* js = Jump on sign                       */
    INST(jne,    BIN(75, 01110101), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jne = Jump on not equal|zero            */
    INST(jnl,    BIN(7d, 01111101), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jnl = Jump on not less|greater or equal */
    INST(jg,     BIN(7f, 01111111), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jg = Jump on not less or equal|greater  */
    INST(jnb,    BIN(73, 01110011), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jnb = Jump on not below|above or equal  */
    INST(ja,     BIN(77, 01110111), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* ja = Jump on not below or equal|above   */
    INST(jnp,    BIN(7b, 01111011), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jnp = Jump on not par|par odd           */
    INST(jno,    BIN(71, 01110001), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jno = Jump on not overflow              */
    INST(jns,    BIN(79, 01111001), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jns = Jump on not sign                  */
    INST(loop,   BIN(e2, 11100010), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* loop = Loop CX times                    */
    INST(loopz,  BIN(e1, 11100001), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* loopz = Loop while zero|equal           */
    INST(loopnz, BIN(e0, 11100000), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* loopnz = Loop while not zero|equal      */
    INST(jcxz,   BIN(e3, 11100011), IP_INC8, _, _, _, _, _, _, _, _, _)                                            /* jcxz = Jump on CX zero                  */

                                                                                                                   /* int = Interrupt: */
    INST(int,  BIN(cd, 11001101), DATA_8, _, _, _, _, _, _, _, _, _)                                               /*   Type specified */
    INST(int3, BIN(cc, 11001100), IMP_DATA_8(3), _, _, _, _, _, _, _, _, _)                                        /*   Type 3         */
